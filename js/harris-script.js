...
(function() {
  ...
  function Article(result, query) {
    this.result = result || {};
    this.query = query || "";
    this.empty = false;
  }
  
  Article.prototype.sigid = function() {
    return this.result._id || "";
  }
  
  Article.prototype.title = function() {
    if (this.query === "ask") {
      return this.result.title.replace(/^Ask HN\: |Ask HN\:|Ask HN - |Ask HN -/i, "");
    } else if (this.query === "show") {
      return this.result.title.replace(/^Show HN\: |Show HN\:|Show HN - |Show HN -/i, "");
    } else {
      return this.result.title;
    }
  }
  
  Article.prototype.hn_text = function() {
    return this.result.text || "";
  }
  
  Article.prototype.domain = function() {
    return this.result.domain || "news.ycombinator.com";
  }
  
  Article.prototype.url = function() {
    return this.result.url || "http://news.ycombinator.com/item?id=" + this.result.id || "";
  }
  
  Article.prototype.points = function() {
    return this.result.points || "0";
  }
  
  Article.prototype.num_comments = function() {
    return this.result.num_comments || "0";
  }
  
  Article.prototype.user = function() {
    return this.result.username || "";
  }
  
  Article.prototype.published_date = function() {
    return Date.fromString(this.result.create_ts).toRelativeTime() || "";
  }
  
  Article.prototype.hn_url = function() {
    return "http://news.ycombinator.com/item?id=" + this.result.id || "";
  }
  
  Article.prototype.hn_user_url = function() {
    return "http://news.ycombinator.com/user?id=" + this.user() || "";
  }
  
  Article.prototype.special = function() {
    return Orange.constants.SPECIAL_DOMAINS.indexOf(this.domain()) !== -1
  }
  
  Article.prototype.content = function() {
    if (this.domain() === "news.ycombinator.com") {
      return this.hn_text();
    } else if (this.domain() === "youtube.com") {
      return '<iframe width="640" height="360" src="http://www.youtube-nocookie.com/embed/' + this.url().split("v=")[1].split("&")[0] + '" frameborder="0" webkitAllowFullScreen allowfullscreen></iframe>';
    } else if (this.domain() === "vimeo.com") {
      return '<iframe src="http://player.vimeo.com/video/' + this.url().split(".com/")[1] + '?title=0&amp;byline=0&amp;portrait=0&amp;color=ff6600" width="640" height="360" frameborder="0" webkitAllowFullScreen allowFullScreen></iframe>';
    }
  }
  ...
  var Orange = {
    ...
    hnsearch: {
      ...
      parse_json: function(results, url, query, start) {
        ...
        while (l--) {
          delete Orange.articles.shift
        }
        Orange.articles = [];
        
        while (i--) {
          Orange.articles.push(new Article(results[i].item, query));
        }
    ...
    listeners: {
      ...
      close: function() {
        ...
        $("nav").on("click", "a.close", function(e) {
          query = $(this).siblings("a").data("search");
    ...
    reader: {
      ...
      render_article: function($this, $target) {
        ...
        els.article.html("<h2 id='article_title'><a href='" + article.url() + "' title='" + article.domain() + "'>" + article.title() + "</a></h2><div id='page_content'>" + article.content() + "</div>");
        
        els.hud.find("a.source").attr("href", article.url()).attr("title", article.domain());
        ...
        if (!article.special()) {
          ...
          } // One of those rare occasions: http://goo.gl/oQY5Y
        }
